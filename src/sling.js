...
async function getCalendar(token, from, to) {
    ...
    const users = conciseUserData.users.map(user => ({
        id: user.id,
        firstname: user.name,
    ...
    // Build groups list
    const groups = Object.entries(conciseUserData.groups).map(([_, group]) => ({
        id: group.id,
        name: group.name
    })).sort(_grpSort);
...
function subDividDays(cal, subDivideBy) {
    const divided = cal.data.reduce((acc, shift) => {
        for (let date = shift.start; date < shift.end; date.setMinutes(date.getMinutes() + subDivideBy)) {
...
function toCSV(data, div) {
    ...
    for (const record of data.data) {
        ...
        const taskColumnIndex = groupNames.indexOf(record.task.name) + firstCols.length;
